...
def get_process_table(plant_code, machine_center_code):
  if ((plant_code==pba_pc) & (machine_center_code==pba_hrm)):
    process_tbl = DB + '.' + pba_hrm_tbl #, base_path + publish_path + env + hrm_folder
    mcc = pba_hrm
  
  elif ((plant_code==pba_pc) & (machine_center_code==pba_hfm)):
    process_tbl = DB + '.' + pba_hfm_tbl#, base_path + publish_path + env + hfm_folder
    mcc = pba_hfm
    
  elif ((plant_code==pba_pc) & (machine_center_code==pba_cm1)):
    #return DB + '.' + pba_cm1_tbl#, base_path + publish_path + env + cm1_folder
    process_tbl = None
    mcc = pba_cm1
    
  elif ((plant_code==pba_pc) & (machine_center_code==pba_cm2)):
    #return DB + '.' + pba_cm2_tbl#, base_path + publish_path + env + cm2_folder
    #process_tbl = DB + '.' + pba_cm2_tbl
    process_tbl = None
    mcc = pba_cm2
    
  elif ((plant_code==pba_pc) & (machine_center_code==pba_cm3)):
    process_tbl = DB + '.' + pba_cm3_tbl#, base_path + publish_path + env + cm3_folder    
    mcc = pba_cm3
...
def get_process_data(plant_code, machine_center_code):
  ...
  if process_tbl==None:
    ...
    process_df = spark.sql(""" SELECT tbl_a.metal_code, tbl_a.eRg_EntryThickness, tbl_a.eRg_ExitThickness, tbl_a.eRg_EntryHeadDiscard, tbl_a.eRg_EntryTailDiscard, tbl_a.eRg_ExitHeadDiscard, tbl_a.eRg_ExitTailDiscard, tbl_a.eRg_EntryRolledLength, tbl_a.eRg_ExitRolledLength, tbl_a.eRg_Ratio, tbl_a.eRg_FlipPriorLength
FROM
(
SELECT metal_code, eRg_EntryThickness, eRg_ExitThickness, eRg_EntryHeadDiscard, eRg_EntryTailDiscard, eRg_ExitHeadDiscard, eRg_ExitTailDiscard, eRg_EntryRolledLength, eRg_ExitRolledLength, eRg_Ratio, eRg_FlipPriorLength, count(*) AS ct 
FROM {} 
GROUP BY metal_code, eRg_EntryThickness, eRg_ExitThickness, eRg_EntryHeadDiscard, eRg_EntryTailDiscard, eRg_ExitHeadDiscard, eRg_ExitTailDiscard, eRg_EntryRolledLength, eRg_ExitRolledLength, eRg_Ratio, eRg_FlipPriorLength
) tbl_a
JOIN
(
SELECT metal_code, MAX(ct) AS max_ct
FROM
(
SELECT metal_code, eRg_EntryThickness, eRg_ExitThickness, eRg_EntryHeadDiscard, eRg_EntryTailDiscard, eRg_ExitHeadDiscard, eRg_ExitTailDiscard, eRg_EntryRolledLength, eRg_ExitRolledLength, eRg_Ratio, eRg_FlipPriorLength, count(*) AS ct 
FROM {} 
...
ON
tbl_a.metal_code=tbl_b.metal_code AND tbl_a.ct=tbl_b.max_ct
...
df121 = spark.sql("SELECT {},{} FROM {}.{} GROUP BY {},{}".format( plant_code, machine_center_desc, DB, coil_lineage_tbl, plant_code, machine_center_desc))
# filter dataframe to the machine centers not present in exit_mcc list
df121 = df121.filter(~col(machine_center_desc).isin(exit_mcc)) # gettign machine centers that are not present in exit_mcc list

# COMMAND ----------

# define an empty dataframe process_df
from pyspark.sql.types import StructType, StructField, LongType, FloatType, BooleanType, StringType
...
cl.alias("cli").merge(process_df.alias("pr"), "cli.metal_code = pr.metal_code AND cli.machine_center_desc = pr.machine_center_desc")\
