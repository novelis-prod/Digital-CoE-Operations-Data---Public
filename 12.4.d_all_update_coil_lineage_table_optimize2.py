...
# MAGIC This notebook will read the process data from respective machine centers and store that in coil lineage table. After storing the data, it will do calculation of some columns. 
# MAGIC 
...
publish_path = 'enterprise/'                                             # change this if environment is changed                                            
env = 'dev/'                                                             # change this if environment is changed
hrm_folder = 'hrm'
hfm_folder = 'hfm'
cm1_folder = 'cm1'
cm2_folder = 'cm2'
...
DB = 'opsentdev'                                                         # change this if DB is changed
...
# MAGIC %sql select * from opsentdev.coil_lineage limit 1
...
hrm_tbl = DeltaTable.forPath(spark, base_path+publish_path+env+hrm_folder)
hfm_tbl = DeltaTable.forPath(spark, base_path+publish_path+env+hfm_folder)
...
cm3_tbl = DeltaTable.forPath(spark, base_path+publish_path+env+cm3_folder)

# COMMAND ----------

# Update HRM
hrm_tbl.alias("hrm").merge(df_hrm.alias("df_hrm"), "hrm.metal_code = df_hrm.metal_code and hrm.sync_length = 0.0 and df_hrm.use_index = eg_LengthIndex" )\
               ...
               .execute()
hrm_tbl.alias("hrm").merge(df_hrm.alias("df_hrm"), "hrm.metal_code = df_hrm.metal_code and hrm.sync_length = 0.0 and df_hrm.use_index = eg_ReverseLengthIndex" )\
               ...
               .execute()
# Update HFM
hfm_tbl.alias("hfm").merge(df_hfm.alias("df_hfm"), "hfm.metal_code = df_hfm.metal_code and hfm.sync_length = 0.0 and df_hfm.use_index = eg_LengthIndex" )\
...
hfm_tbl.alias("hfm").merge(df_hfm.alias("df_hfm"), "hfm.metal_code = df_hfm.metal_code and hfm.sync_length = 0.0 and df_hfm.use_index = eg_ReverseLengthIndex" )\
...
cm1_tbl.alias("cm1").merge(df_cm1.alias("df_cm1"), "cm1.metal_code = df_cm1.metal_code and cm1.sync_length = 0.0 and df_cm1.use_index = eg_LengthIndex")\
               ...
               .execute()
cm1_tbl.alias("cm1").merge(df_cm1.alias("df_cm1"), "cm1.metal_code = df_cm1.metal_code and cm1.sync_length = 0.0 and df_cm1.use_index = eg_ReverseLengthIndex")\
...
"""
# Update CM2
cm2_tbl.alias("cm2").merge(df_cm2.alias("df_cm2"), "cm2.metal_code = df_cm2.metal_code and cm2.sync_length = 0.0 and df_cm2.use_index = eg_LengthIndex")\
               ...
               .execute()
cm2_tbl.alias("cm2").merge(df_cm2.alias("df_cm2"), "cm2.metal_code = df_cm2.metal_code and cm2.sync_length = 0.0 and df_cm2.use_index = eg_ReverseLengthIndex")\
...
# Update CM3
cm3_tbl.alias("cm3").merge(df_cm3.alias("df_cm3"), "cm3.metal_code = df_cm3.metal_code and cm3.sync_length = 0.0 and df_cm3.use_index = eg_LengthIndex")\
               ...
               .execute()
cm3_tbl.alias("cm3").merge(df_cm3.alias("df_cm3"), "cm3.metal_code = df_cm3.metal_code and cm3.sync_length = 0.0 and df_cm3.use_index = eg_ReverseLengthIndex")\
...
# MAGIC %sql select * from opsentdev.coil_lineage where metal_code=1000000000000006 order by ops_seq