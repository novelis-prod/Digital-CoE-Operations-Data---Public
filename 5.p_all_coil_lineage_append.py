...
#query to get highest metal code value in coil lineage table
query_metal_code = 'select metal_code from ' + DB + '.'+ coil_lineage_tbl+' order by metal_code desc limit 1'
metal_code_highest = spark.sql(query_metal_code).collect()[0][0]
...
query = 'select metal_code,plant_code,parent_coil_id,coil_id,operation_start_date,operation_end_date,machine_center_code from ' + DB_MES + '.'+ mes_tbl +' where metal_code>'+str(metal_code_highest)
...
#create a column ops_seq to assign a number based on flow of operation seq
...
coil_lineage_df.repartition(1).write.format('delta').mode('append').option('path',destination_path_coil_lineage).saveAsTable(DB+'.'+tbl_prod_coil_lineage)