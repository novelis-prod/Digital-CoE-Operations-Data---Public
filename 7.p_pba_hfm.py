...
# MAGIC This notebook is to read the files from file log table and create HFM delta tables for plant pinda.
# MAGIC 
# MAGIC • Get all the files in the file log delta table which are not yet read(is_read=0) and machine center value as HFM
# MAGIC 
# MAGIC • Read all those files and store it in HFM delta table
# MAGIC 
# MAGIC • Run the notebook for every X minutes to update the delta table with new data
# MAGIC 
# MAGIC Input: All unread files of machine center HFM
# MAGIC 
# MAGIC Output: Store the new data in HFM delta table
