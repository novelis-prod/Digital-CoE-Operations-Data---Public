...
# MAGIC 
# MAGIC This notebook is to read raw data of HRM/HFM machine centers from landing zone and store it as delta tables in enterprise zone.
# MAGIC 
# MAGIC • Read the data of HRM/HFM machine center files from file log delta table and drop the rows having null values in every column
# MAGIC 
...
# MAGIC 
# MAGIC • Update some specific column values with zeros for NaN values and round the remaining values to 2 decimal places
# MAGIC 
...
#input path to read data for MES  table
...
x=input_path.split('/')
df =df.withColumn('metal_code', lit(metal_code_default))\
      .withColumn('plant_code', lit(plant_code))\
      .withColumn('filename',lit((x[-1].split('.'))[0]))\
...
hm_df =df.withColumn('Time', col('Time').cast('timestamp'))

# COMMAND ----------

try:
  #query to get data from HRM/HFM delta table
  query = 'select * from ' + DB +'.'+ table_pinda_hm + ' limit 1'
  hm_df_rec = spark.sql(query)                                      #define a dataframe with HRM/HFM delta table data
  ...
  count = 0
  for i in hm_df_rec.columns:
    if i not in hm_df.columns:
      count=count+1
  
  try:
    if count>1 and len(hm_df.columns)!=len(hm_df_rec.columns):
      file_log_tbl.update(col("file_path") == input_path,{"error_code": lit('Schema mismatch')})
      dbutils.notebook.exit('Schema mismatch')
  except:
    print('error in updating file log')    

# COMMAND ----------

#store data into a delta table
hm_df.repartition(1).write.format("delta").mode("append").partitionBy('year','month','day').option('path',destination_path).saveAsTable(DB+'.'+table_pinda_hm)
